...
## Build step 9 -- SAFETY.md, REASON_CODES.md, results summary, README polish

### SAFETY.md (F8)

Written from what we actually know, not a template: what the defense
protects against (grounded in specific rule ids), what data it observes
(grounded in the wire contract and the guard.py structural exclusion),
when a human is consulted, expected false-positive behaviour, and -- the
two sections that matter most for honesty -- "Known failure modes" (the
four real bugs found across build steps 4/6/7, restated as safety-relevant
findings rather than buried in FIXLOG) and "What this does not protect
against" (insider-trusted instructions, semantic goal drift within policy,
paraphrase evasion, an attack indistinguishable from genuine content in
trust/form/content -- stated plainly, not hedged).

### REASON_CODES.md (F7's "closed, documented taxonomy")

Every reason code the system can emit, generated by grepping the actual
source (`config/rules/*.yaml`, `config/likelihoods.yaml`, and the hardcoded
strings in `stage3_decide.py`/`adapter.py`), not typed from memory.
`tests/test_reason_codes_documented.py` keeps this honest going forward: it
loads the same rule/signal specs the running system uses and fails if any
reason code isn't listed in the file -- documentation that can't silently
drift out of date the way a hand-maintained list could.

### Baseline comparison (report template's explicit ask)

The technical report template requires "baselines compared: at least
allow_all, provenance, and one other" -- not yet produced before this step.
Ran all five shipped baselines (`allow_all`, `deny_sensitive`, `keyword`,
`heuristic_risk`, `provenance`) against the public split
(`results/baselines/*.json`) alongside our own. Findings worth having found:
`keyword`/`deny_sensitive` fail ASR exactly as the spec predicts for naive
defenses; `provenance` -- the baseline our method most directly extends --
reaches ASR 0.0 like we do but has **FBR 0.046** where we have **FBR
0.000**, which is the first *quantified* evidence (not just our own
before/after ablation numbers) that the encoding-aware, most-trusted-
explanation provenance tracer built in this repo genuinely improves on the
shipped baseline's verbatim-text-overlap approach, on precision, not just
robustness.

Files: `results/baselines/*.json`, `results/RESULTS_SUMMARY.md` (consolidated,
paste-ready: main table, per-domain/per-family breakdown, ablation summary,
calibration summary, failure-analysis pointers into this file).

### README polish

Added a "Results at a glance" section and a documentation index near the
top, ahead of the step-by-step build log -- the log stays (it's real
evidence: every step's tests, metrics, and bugs found), but a judge opening
the repo cold now sees the headline comparison table and a map of where
everything lives before the chronological detail.

### Status

135/135 tests passing. Every required-by-brief document now exists:
SAFETY.md, REASON_CODES.md, README.md, FIXLOG.md, a results section ready
to paste into the report. Remaining before submission: build step 10
(re-validate against `--model qwen3-8b`, record the three demo traces the
video is built around), the video itself, and the technical report
document (this repo's markdown files are the source material for it, not a
substitute for it).
